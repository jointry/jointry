 ...
 * Base64エンコード／デコード
 * <p>
 * Base64 はバイナリデータを 64個の印刷可能ASCII文字(A-Za-z0-9+/)で表現する 符号化方式。3バイト（24ビット）を 4個の
 * 6ビットに分け、各6ビットデータを 64個の印刷可能文字にマップすることで、バイナリデータを印刷可能文字からなる
...
public class Base64 {
    ...
    /**
     * 指定されたファイルのBase64エンコード文字列を取得する。
     *
     * @param infile ファイルのパス名
     * @return Base64エンコード結果の文字列
     */
    public static String encode(String infile) {
        StringWriter writer = new StringWriter();
        InputStream is = null;
        try {
            is = new BufferedInputStream(new FileInputStream(infile));
            encode64(is, new BufferedWriter(writer));
        } catch (IOException e) {
    ...
    /**
     * Base64エンコード文字列をデコードしファイルに保存する。
     *
     * @param text Base64エンコード文字列
     * @param outfile ファイルのパス名
     */
    public static void decode(String text, String outfile)
            throws IOException {
    ...
    static class TextBlockReader implements LineReader {
        ...
        TextBlockReader(List<String> textBlock) {
            this.textBlock = textBlock;
        }

        @Override
        public String readLine() {
            if (lno >= textBlock.size()) {
                return null;
            }
