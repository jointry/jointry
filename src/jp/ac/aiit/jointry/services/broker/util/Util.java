...
public class Util {
	...
	/**
	 * テキストファイルから文字列配列のリストを作る。
	 * テキストファイルの各行は、１個以上の空白文字で分けられた項目からなり、
	 * 文字列配列配列に変換される。
	 * @param fname ファイル名
	 * @return 文字列配列のリスト
	 */
	...
	public static String readText(File file) {
		try { return readTextEx(file); } catch(IOException e) {
			e.printStackTrace();
			return "";
		}
	}
	...
	private static String readTextEx(BufferedReader reader) throws IOException {
		...
		String line;
		while((line = reader.readLine()) != null) sb.append(line + CRLF);
		reader.close();
		return sb.toString();
	}
	...
	public static String imageSuffix(String fname) {
		...
		if(suffix.equals("jpg") || suffix.equals("png")) return suffix;
		return null;
	}
	...
	public static int timeOffset() { return timeOffset; }
	public static void timeOffset(int offset) { timeOffset = offset; }
	...
	private static int findIndex(String unit) {
		for(int i = 0; i < TIMEUNITS.length; i++) {
			if(TIMEUNITS[i].equals(unit)) return i;
		}
