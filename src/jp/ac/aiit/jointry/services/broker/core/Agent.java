 ...
 * メッセージ送信、同期呼出し、QUERY送信、NOTIFY送信、テキスト送信、画像送信、
 * 画像ファイル送信などを行うことができる。
 * <ul>
 ...
 * </ul>
 * 
 * Agentの親クラスHttpBaseで定義された以下のメソッドをオーバライドすることに
 * よって、各種のメッセージの受信時の処理を拡張することができる。
 * <ul>
...
public class Agent extends HttpBase implements Evaluator {
	...
	public Agent() {}
	public Agent(Monitor monitor) { setMonitor(monitor); }

	private char roleMark = 'u';
	private void setRoleMark(String role) {
		roleMark = SERVER.equals(role) ? 's' : CLIENT.equals(role) ? 'c' : 'u';
	}
	@Override public char mark() { return roleMark; }
	@Override public boolean isAgent() { return true; }
	@Override public String continuousDialogID() { return getProp(K_PROXY); }
	...
	/**
	 * Proxyからのメッセージ受付を開始する。
	 * オープニングメッセージを表示し、受信待ち状態になる。
	...
	/**
	 * Proxyからの交信終了の通知を受け呼び出される。
	 * Agent側ではソケットを利用不可状態としモニタに終了の通知を出す。
	 */
	...
	public DInfo query(String... keys) {
		...
		for(String key : keys) dinfo.set(key, null);
		return invoke(dinfo);
	}
	...
	/** 宛先を管理するリストを返す。 */
	public List<String> getDestinationList() { return destList; }

	/** 宛先を管理するリストからすべての宛先を削除する。 */
	public void clearDestinationList() { destList.clear(); }
	...
	public void initDestinationList() {
		...
		for(String cp: members) {
			if(!cp.equals(proxyID)) destList.add(cp);
		}
	}
	 ...
	 */
	public void addDestination(String proxyID) { destList.add(proxyID); }

	/**
	 * Proxyを削除する。
	 * @param proxyID 削除するProxyの識別子
	 */
	public void removeDestination(String proxyID) { destList.remove(proxyID); }

	/**
	 * 送信プロパティ @to に配布先の宛先を設定し、NOTIFYを送信する。
	 * すでに明示的に宛先(@to)が指定されていたらそれを採用する。
	 * Agentの宛先管理リストが定義されていたらその内容を@toに設定する。
	 * それ以外の場合は未定義とし、Broker側（Proxy/ClientProxy/ServerProxy）の
	 * 配布ポリシーに従う。
	 * @param dinfo 送信するNOTIFY情報
	 */
	@Override public void notifyD(DInfo dinfo) {
	...
	private void evalDest(String param) {
		if(param != null) {
			...
			if(param.equals("clear")) {
				...
				if(head != '+' && head != '-') destList.clear();
				for(String pid : pids) {
					switch(head) {
					case '-': removeDestination(pid); break;
					default: addDestination(pid); break;
					}
				}
			}
		}
	 ...
	 * プロパティのヘルプメッセージを表示する。
	 */
	@Override public void printPropHelp() {
	 ...
	 */
	@Override public boolean evalProp(String[] tt) {
		if(tt.length == 0) return false;
		String tag = tt[0];

		if(tag.equals("timeout")) {
			if(tt.length == 2) setInvokeTimeout(Util.getInt(tt[1]));
