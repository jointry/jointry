 ...
 * メッセージの転送やコンテンツの処理に必要な情報（コンテンツ情報、 コンテンツの表示制御、中継点・終端区別）を管理する。
 * <p>
...
public class DInfo extends BPInfo {
    ...
    /**
     * メッセージの受信者がコンテンツをモニタに表示するか否かを判定するフラグ。 メッセージの受信者は、コンテンツの種別に応じて、Monitorが提供する
     * viewText(title, text)、viewImage(title, bimage)、println(msg)などの
     * メソッドを使用して、コンテンツを表示することができるが、 visibleを使ってこの表示を制御できる。
    ...
    /**
     * メッセージ受信時に通信の終点の場合はtrueとする。 受信側オブジェクトがServerProxyかClientProxyならばfalse、
    ...
    /**
     * テキストコンテンツを文字列形式で返す。 受信したテキストファイルはプレフィックスとして`@'文字を付けた
    ...
    /**
     * リーダからテキストを読み込み、ファイルに保存し、そのファイル名を返す。 読み込むテキストの終了はEOF(空白行)で判定する。
     *
    ...
    /**
     * ソケットのリーダから複数行のテキストを読み込み、文字列Listとして返す。 読み込むテキストの終了はEOF(空白行)で判定する。
     *
    ...
    /**
     * データコンテンツをファイルに書き出す。 Base64でエンコードされたデータコンテンツはデコードしファイルに書き出す。
     *
    ...
    /**
     * 画像コンテンツから画像データを再現しその結果を返す。 画像コンテンツは、contentフィールドにその内容を保持するか、または
    ...
    public BufferedImage getImageContent() {
        if (content != null) {
            return ImageUtil.createImage(content);
        }
        if (contentFile != null) {
            return ImageUtil.createImage(contentFile);
        }
        return null;
    }
