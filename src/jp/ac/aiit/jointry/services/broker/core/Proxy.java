 ...
 * 省略した場合はProxyグループの情報を元にProxyまたはその拡張クラスが
 * メッセージ配送ポリシーに従って配送先を特定する。
 * 配送方法は、メッセージの発信元種別（クライアント/サーバ）と
 * メッセージ種別(QUERY/NOTIFY/COMMAND/PLAIN)によって決まる。
 * <pre>
 * ------------------------------------------------
 ...
 * ダイアログ情報の@fromプロパティに発信元のProxy(発信元のAgentに対応するProxy)
 * のproxyIDを転送の際に追加している。
 * 
 * @see Broker
 * @see Agent
 * @see DInfo
...
public class Proxy extends HttpBase {
	 ...
	 * Proxy の一意識別子を取得する。
	 */
	public int getID() { return proxyID; }

	/**
	 * proxyIDの文字列表現を持続的ダイアログのIDとして返す。
	 */
	@Override protected String continuousDialogID() { return ""+proxyID; }

	/**
	 * このProxy のユーザ名を得る。
	 */
	public String getUserID() { return httpInfo.get(USER_ID, UNKNOWN_USER); }
	...
	@Override public char mark() { return 'P'; }
	...
	/**
	 * Agentから送信されたQUERYの受信処理を行う。
	 * K_TOプロパティに従って宛先のProxyを特定し転送する。
	...
	/**
	 * Agentから送信されたANSWERの受信処理を行う。
	 * K_FROMの内容に従って返信先を特定し、転送処理を行う。
	 * @param dinfo 受信したANSWER情報
	 */
	@Override protected void answerReceived(DInfo dinfo) {
	 ...
	 */
	@Override protected void onCommand(String command) {
		println(getUserID() + "> " + command);
		sendText(broker.evalShell(command));
	}

	/**
	 * 受信したメッセージにユーザIDを付加したメッセージを
	 * Brokerに接続中の全てのProxyに配信する。
	 */
	...
	/**
	 * Agentからの交信終了の通知を受け呼び出される。
	 * Proxy側ではソケットをクローズしBrokerとの接続を解消する。
	 */
	@Override protected void onClose() {
		close();
		if(broker != null) broker.removeProxy(this);
