 ...
 * Agent、Dialogの共通のベースクラスである。
 * これらのコンポーネントはSocketで接続された通信相手とメッセージの交換を行う。
 * すなわち相手側からのメッセージを待ちながら相手側へメッセージを送ることが
 * できなければならない。二つのスレッドに送信と受信を分担させる必要がある。
 * そこでWorkerBaseにはThreadを継承させ相手側からのメッセージを待ち受ける
 * 役割をもたせた。
 * <p>
 * 双方向通信を行うアプリケーションでは各種データコンテンツの送受信が
 * 発生する。WorkerBaseでは、送信するデータコンテンツの配置と
 * 受信するデータコンテンツの配置のために docRoot と workRootの
 * 二つのディレクトリを管理している。
 * <p>
 * 双方向通信のアプリケーション開発のテスト段階では、
 * 送受信されるメッセージのシーケンスを容易に追跡できることが必要となる。
 * NSBrokerでは、通信基盤の内部の状態や受信したメッセージをアプリケーション側に
 * 伝える仕組みとしてMonitorインタフェースを定義した。
 * WorkerBaseは、Monitorインタフェースを実装したMonitorクラスを保持し、
 * このメソッドを呼出すAPIを提供している。
 * <p>
...
public class WorkerBase extends Thread implements Common {
	 ...
	 * 現在のクラスのクラス名を返す。
	 */
	protected String theClass() { return this.getClass().getSimpleName(); }

	/**
	 * ドキュメントのルートディレクトリ：
	 * HTTP GET や viewダイアログで送信するファイルはこのディレクトリに置かれる。
	 */
	...
	public void setDocRoot(String docRoot) { this.docRoot = docRoot; }

	/**
	 * 作業用のルートディレクトリ：
	 * Broker, Proxy, Agent がファイルを保存する際に使用する。
	 * それぞれで再定義しファイルを生成する Dialogに引き継がなければならない。
	 */
	protected String workRoot = "./workroot";

	/**
	 * 作業用のルートディレクトリの下に指定されたファイル名のパス名を作る。
	 * ファイルが生成される位置はworkRoot 内に制限される。
	 * @param fname ファイル名
	 * @return ファイルのパス名
	...
	protected String createImageFile(String title, BufferedImage bimage) {
		try {
			if(Util.suffix(title).isEmpty()) title += ".jpg";
			String filepath = makeFilePath(title);
			if(ImageUtil.writeBufferedImage(filepath, bimage)) return filepath;
		} catch(IOException e) {
	 ...
	 * MonitorAPIの呼出し
	 */
	public void msglog(boolean msglog) { monitor.msglog(msglog); }
	protected boolean msglog() { return monitor.msglog(); }
	public void print(String msg) { monitor.print(msg); }
	public void println(String msg) { monitor.println(msg); }
	protected void showStatus(String status) { monitor.showStatus(status); }
	protected void showError(String status) {
		showStatus("★ " + status);
		printError(status);
	}
	protected void printError(String msg) { println("ERROR: "+msg); }
	 ...
	 * 入出力メッセージのログ表示用API
	 */
	protected void logRead(String log) { println(">>>\t" + log); }
	protected void logSend(String log) { println("<<<\t" + log); }
	...
	protected boolean printStackTrace(Throwable t, String msg) {
		//t.printStackTrace();
		if(msg == null) msg = "Exception/Error thrown";
	...
	protected boolean probe(String msg) {
		if(msg == null) msg = "Debug.probeStack";
		println(Util.date() + " " + msg);
	...
	protected boolean time(String tag) { return time(tag, null); }
