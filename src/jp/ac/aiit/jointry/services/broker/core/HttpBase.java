 ...
 * 待ち受ける処理をWorkerBaseから継承するスレッドに担当させている。
 * メッセージ待ち受け処理は、通信相手から送信されるメッセージに対して、
 * メッセージの種別に応じたメソッドを呼び出す。
 * メッセージは、QUERY, ANSWER, NOTIFY, COMMAND, PLAIN の5種類がある。
 * <p>
 * QUERYは ANSWERの返信を待つメッセージであり、
 * 正常に処理されるとANSWERメッセージが返信される。
 * NOTIFYは一方向の送信を行うメッセージであり送達確認を必要としない。
 * COMMANDはShellコマンドとして評価され、その結果標準出力へ出力されたテキストが
 * テキストコンテンツとして返される。
 * PLAINはメッセージとしての空白行をデータコンテンツの区切りを表す空白行と
 * 区別するために導入した。
 * <p>
 * 各メッセージはそれぞれ先頭の１文字によって区別される。
 * `?'はQUERY、`%'はANSWER、`!'はNOTIFY、`>'はCOMMAND、`@'はPLAINと解釈される。
 * それ以外の文字のメッセージが連続する場合は最初のメッセージ以外は
 * 通信不全による雑音として読み飛ばされる。
 * <p>
 * Brokerは QA型通信とNOTIFY型通信の２種類の通信形式を提供している。
 * これらのAPIはDialogインタフェースで定義されている。
 * QA型通信は、双方向通信のための基本機能として、QUERYの送信、
 * QUERYの受信とANSWERの返信、ANSWERの受信を行うAPIを提供する。
 * １つのQUERYの送信後は、そのQUERYのANSWERを受信し処理が完了するまで
 * 新たなQUERYを送信することはできない。
 * HttpBaseはANSWERの受信待ち状態を管理するために送信したQUERYの情報を保持する。
 * NOTIFY型通信は送達確認を行わない通信方式であり、NOTIFYの送信とNOTIFYの受信を
 * を行うAPIを提供する。
 * <p>
...
public class HttpBase extends WorkerBase {
	...
	protected HttpBase() {}
	protected HttpBase(Socket socket) { setSocket(socket); }

	/** HttpBaseの拡張クラスの種別を表す文字 */
	public char mark() { return 'H'; }
	...
	public void clearDialogs() {
		if(continuousDialogs == null) return;
		for(String did : continuousDialogs) DialogBase.removeDialog(did);
	 ...
	 * 持続的ダイアログのIDを返す。
	 */
	protected String continuousDialogID() { return "0"; }

	/**====== HTTPプロトコルでの通信の確立と解除のためのAPI =======<br>
	 * URLを使った通信の確立、ソケットの設定、他のHttpBaseオブジェクトからの
	 * 実行環境の引き継ぎ、通信の解除等を行う。
	 */

	/**
	 * URLInfoのホストとポートを使ってソケットを生成し、そのソケットの
	...
	/**
	 * 他のHttpBaseオブジェクトから実行環境を引き継ぐ。
	 * 引き継ぐ情報としては、実行監視モニタ、ソケット、入力／出力ストリーム、
	...
	/**
	 * HTTP リクエストを発行する。主にAgent が使用する。
	 * 発行したHTTP リクエストをモニタに表示することができる。
	 * @param method メソッド名
	...
	synchronized void sendRequest(String method, Map<String,String> paramMap,
		...
		if(msglog()) println("*** " + theClass() + " sends a request:");
	...
	/**
	 * 要求メッセージを読み取り、HttpInfoとして返す。主にProxyが使用する。
	 * HttpInfo には、メソッドとURI、メッセージのヘッダとボディが含まれる。
	 * 応答メッセージをモニタ表示できる。
	 * @return 読み取った要求メッセージ
	...
	protected List<String> receiveHTTP(int timeout, String message)
		...
		if(line == null) throw new IOException("no message");
		...
		if(msglog()) {
			...
			for(String s : contents) logRead(s);
		}
		return contents;
	}
	 ...
	 * データ種別やファイル名等の属性情報をメッセージとして送信し、
	 * それに続いてテキストデータや画像データ等のコンテンツデータを送信する。
	 * <p>
	 * テキストデータは各行を1つのPLAINとして連続して送信する。
	 * 画像データはBase64でエンコードしテキストデータとして送信する。
	 * データの終了は空白行とする。
	 * <p>
	 * コンテンツデータの受信はこの逆の過程をとる。
	 * 即ちメッセージ受信後、コンテンツデータを空白行まで読み込み
	 * 文字列リストを生成する。その後は受信メッセージに含まれる属性情報から
	 * 元のコンテンツを再現する。
	 */
	...
	/**
	 * テキスト（改行コードを含む文字列）を送信する。
	 * テキストの各行の行頭にはMS_PLAIN('@')をつける。
	 * 最終行の後に空白行を送信する。
	...
	/**
	 * NOTIFYによる画像データの送信処理：
	 * 画像データをjpg圧縮し、Base64でエンコードして送信する。
	 * @param title データタイトル
	 * @param bimage 送信する画像データ
	 * @return 送信に成功したらtrue
	 */
	synchronized
	public boolean notifyViewImage(String title, BufferedImage bimage) {
	...
	/**================ メッセージ受信のためのAPI =================<br>
	 * メッセージ受信スレッドを起動し、メッセージの受付とメッセージ種別に
	 * 応じたディスパッチを行う。
	 */
	...
	/**
	 * メッセージ受付を終了した時に呼び出される。
	 * 拡張クラスでは、このメソッドをオーバライドすることによって
	 * セッション終了時の処理を記述できる。
	 */
	...
	protected void handle() throws IOException {
		...
		while((line = reader.readLine()) != null) {
			if(msglog()) logRead(line);
			if(line.isEmpty()) continue;
			if(socket != null) socket.setSoTimeout(RESPONSE_TIMEOUT);
			switch(line.charAt(0)) {
			case M_QUERY:   queryReceived(line.substring(1));   break;	// ?
			case M_ANSWER:  answerReceived(line.substring(1));  break;	// %
			case M_NOTIFY:  notifyReceived(line.substring(1));  break;	// !
			case M_COMMAND: onCommand(line.substring(1));       break;	// >
			case M_PLAIN:   onMessage(line.substring(1));       break;	// @
			default:        onMessage(line); skipFollowers();   break;
			}
			if(socket != null) socket.setSoTimeout(PROXY_TIMEOUT); // ★
		}
	}

	/**
	 * QUERYを受信しANSWERを送信する。
	 * <p>受信した QUERYの正当性を確認し、正しい QUERYに対して ANSWERを返信する。
	 * 不正な QUERYに対してはエラーの種別を返信する。
	...
	/**
	 * 受信済みの後続するメッセージを読み飛ばす。
	 * 本通信方式では、メッセージは１行単位で断続的に送信されるものとしている。
	 * 連続するメッセージは、メッセージの読みこぼしなどの送受信処理の失敗により、
	 * コンテンツ(データ本体)が流出したものと考えられる。
	...
	private void skipFollowers() throws IOException {
		...
		if(count != 0) println(Util.date()+"\t"+count+" mesasge(s) skipped.\n");
	}
	 ...
	 */
	public void syncMyTime() { PingDialog.setTimeOffset(-getTimelag()); }
	...
	public int getTimelag() {
		...
		for(int i = 0; i < 5; i++) {
			...
			if(onanswer == query) return query - onquery;
	 ...
	 * QUERYの発行後は対応するANSWERが返信されるまで次のQUERYを
	 * 発行できないように制御される。
	 * <p>
	 ...
	 * すなわちQUERY発行後は即座に制御が呼出し元に返る。
	 * アプリケーション（あるいはそのユーザが）ANSWERの返信を待たずに
	 * 次のQUERYを発行するとエラーとなる。
	 * <p>
	...
	public void setInvokeTimeout(int timeout) { invokeTimeout = timeout; }
	public int getInvokeTimeout() { return invokeTimeout; }

	/**
	 * ANSWER待ちのQUERYをリセットする。
	 * ANSWERを取りこぼすと次のQUERYの発行ができなくなる。
	...
	synchronized public boolean invokeQuery(String query) {
		...
		if(answer != null) {
			if(returnFlag) println(answer.params("\t= ", "\n"));
	...
	synchronized public boolean invokeShell(String command) {
		...
		if(!content.isEmpty()) println(content);
		if(!exitCode.equals("0")) println("exitCode: " + exitCode);
		if(returnFlag) println(answer.params("\t= ", "\n"));
		return true;
	}
	...
	synchronized public boolean invokeScript(String script) {
		...
		String content = answer.get(K_CONTENT);
		if(content != null && !content.isEmpty()) println(content);
		if(returnValue != null) println("=> " + returnValue);
		if(returnFlag) println(answer.params("\t= ", "\n"));
		return true;
	}
	...
	public DInfo invoke(DInfo dinfo) {
		...
		if(!queryD(dinfo)) return null;
		invokerThread = Thread.currentThread();
		synchronized(invokeLock) {
			boolean invoked = false;
			try { invokeLock.wait(invokeTimeout);
			} catch(InterruptedException e) {
				invoked = true;
			}
			if(!invoked) printError("invoke timeout");
		}
		invokerThread = null;
		return answerInfo;
	}
	 ...
	 * <p>
	 * ANSWER待ちのQUERYがなければ、ANSWERの受信受付の準備を行い、
	 * QUERYを発行する。不正な QUERYに対してはエラーメッセージを表示する。
	 * @param query 送信するQUERYメッセージ
	...
	/**
	 * 送り状(NOTIFYメッセージ)ダイアログ情報を作りの送信処理を実行する。
	 * 送達確認は行わない。
	 * 不正な送り状に対してはエラーメッセージを表示する。
	 * NOTIFYの送信は以下の書式に従う。
	 * @param invoice 送り状
	 * @return エラーの場合false
	 */
	 ...
	 * ダイアログ情報に基づいて動的にダイアログオブジェクトを生成し
	 * Dialogインタフェースで定義されたメソッドを呼び出す。
	 * ダイアログ情報には、ダイアログ識別子、プロパティリスト、
	 * コンテンツ情報、ダイアログの制御情報等が含まれる。
	 */
	/**
	 * ダイアログ情報に基づきQUERYの送信処理を呼び出す。
	 * 送信するQUERYには連番を振る。この連番は受信時に確認される。
	 * @param dinfo ダイアログ情報
	 */
	synchronized private boolean queryD(DInfo dinfo) {
		...
		if(!result) queryInfo = null;
		return result;
	}

  	/**
	 * ダイアログ情報に基づきQUERYの受信処理を呼び出す。
	 * 拡張クラスでこのメソッドを再定義し、他のProxyへの転送処理などを
	 * 行うことができる。
	 * @param dinfo ダイアログ情報
	 */
	...
	/**
	 * ダイアログ情報に基づきNOTIFYの送信処理を呼び出す。送達確認は行わない。
	 * 送信プロパティで宛先を設定したい場合は、
