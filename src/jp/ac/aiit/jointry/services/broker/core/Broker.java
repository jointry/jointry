...
import jp.ac.aiit.jointry.services.broker.core.Shell;
import jp.ac.aiit.jointry.services.broker.core.Proxy;
import jp.ac.aiit.jointry.services.broker.core.ProxyFactory;
...
/**
 * Brokerの双方向通信の概念モデルは、 Broker, Proxy, Agentの３種類の構成要素からなる。
 * Brokerはサービス交換の場を提供する役割をもつ。 AgentはBrokerに対してサービス交換のために接続要求を出す。
 * BrokerはAgentからの接続要求を受けたら、そのAgentに対応するProxyを 生成しこのProxyにAgentとの双方向通信を委譲する。
 * <p>
 * 一般的なサービスの交換形態では、サービスを提供する１つのAgent(SA)が、 サービスを利用する複数のAgent(CAi)と１対多の関係をもつ。
 * サービスを提供するAgentをサーバ、サービスを利用するAgentをクライアントと呼ぶ。
 * Broker(B)の中では各Agentに対応するProxyがサービス交換の実務を担当する。
 * SAとCAi(i=1..n)に対応して、SPとCPiが作られる。 これらがProxyのグループを形成しグループでの双方向通信が可能になる。
 * SPとCPi(i=1..n)をあわせてProxyグループと呼ぶ。 Brokerは複数のProxyグループを管理し、
 ...
 * </pre>
 * <p>
 * Brokerの実装の概要を説明する。 Brokerは、Agentに対する接続サービスを提供し利用状況を管理するために、
 * Agentのアカウントを管理している。 Brokerは、起動後直ちにServerSocketでAgentからの接続要求を待ち受ける。
 * Brokerは接続要求を受理したら適切なProxyを生成し、 Agentとの交信を開始させるために生成したProxyを起動する。
 * <p>
 * Brokerは、現在Agentに接続され稼働している全てのProxyのリストと、 全てのServerProxyのリストをもち、
 * 様々な形態のProxyグループの管理を可能にしている。 アカウント情報とProxyグループの情報を用いて 様々な接続形態のサービス交換が可能になる。
 * <p>
 * Proxyを生成するメソッドは様々な接続形態のサポートが実装できるように 設計してある。
 * 現在の実装では、ServerProxyの二重起動や不在時の代替Proxyへの接続、
 * 同一クライアントの同時接続等の制御をフラグで行う仕組みを組み込んである。
 * <p>
 * クライアントからの接続要求に対して接続すべきサーバを特定することは マッチング戦略に係る問題である。
 * 現在の実装ではServerProxyのリストから最初にサービス可能なものを返す。 このメソッドをオーバライドすることによって、マッチング戦略を
 * カスタマイズすることができる。
 * <p>
 * Brokerは、稼働中のProxyに対して各種のメッセージやデータコンテンツを ブロードキャストするためのAPIを提供している。
 *
 * @see Proxy
 * @see Agent
 * @see HttpInfo
...
public class Broker extends WorkerBase implements Shell {
    ...
    /**
     * コマンドラインパラメータを使って、 サーバソケットやドキュメントルートの初期化を行う。
     *
     * @param args コマンドラインパラメータ
     */
    ...
    /**
     * 交信を終了するProxyをアカウントから削除する。 ProxyがAgentからの交信終了通知を受けた場合に呼び出される。
     *
     * @param name ユーザID
     */
    ...
    public void multiServer(boolean ms) {
        multiServer = ms;
    }
    ...
    public void multiClient(boolean mc) {
        multiClient = mc;
    }
     ...
     */
    public Broker() {
        proxyFactory = new ProxyFactory(this.monitor);
    }
     ...
     */
    public List<ServerProxy> spList() {
        return spList;
    }
     ...
     * currProxyIDのプロキシが存在する場合はそれをinvokeする。 存在しない場合は、Brokerに接続しているプロキシを順番に呼出す。
    ...
    public void setInvokeTimeout(int t) {
        timeout = t;
    }

    public int currProxyID() {
        return currProxyID;
    }

    public void currProxyID(int id) {
        currProxyID = id;
    }
    ...
    /**
     * クライアントからの接続要求に対して接続すべきサーバを特定する。 現在の実装ではServerProxyのリストから最初にサービス可能なものを返す。
     * このメソッドをオーバライドすることによって、マッチング戦略を カスタマイズすることができる。
     *
     * @param hinfo 接続情報
    ...
    /**
     * サーバ起動時の処理を記述するハンドラ。 拡張クラスでオーバライドできる。
     *
     * @param hinfo 接続情報
     */
    ...
    /**
     * クライアント起動時の処理を記述するハンドラ。 拡張クラスでオーバライドできる。
     *
     * @param hinfo 接続情報
     */
     ...
     * Brokerは、運用状況の管理のために、稼働中のProxyに対して 各種のメッセージやデータコンテンツをブロードキャストするための
     * APIを提供している。
     */
    ...
    synchronized public void pushMessage(String message) {
        for (Proxy proxy : proxyList) {
            proxy.sendMessage(M_NOTIFY + message);
        }
    }
    ...
    synchronized public void pushText(String title, String text) {
        for (Proxy proxy : proxyList) {
            proxy.notifyViewText(title, text);
        }
    }
